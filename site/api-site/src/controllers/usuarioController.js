...
function cadastrarArtes(req, res) {
    ...
    if (id == undefined) {
        ...
        res.status(400).send("Sua seleçao está undefined!");
    } else {
        
        usuarioModel.cadastrarArtes(id, selecao)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
...
function enviarGrafico(req, res) {
    ...
    if (id == undefined) {
        ...
        res.status(400).send("Sua voto está indefinida!");
    } else {
        
        usuarioModel.enviarGrafico(id, voto)
            .then(
                function (resultado) {
                    console.log(`\nResultados encontrados: ${resultado.length}`);
                    console.log(`Resultados: ${JSON.stringify(resultado)}`); // transforma JSON em String
